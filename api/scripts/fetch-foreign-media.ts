...
const OUTPUT_JSON_FILENAME = 'foreign-media-news-2025-08-18.json';
...
function formatNewsRecord(record: FeishuNewsRecord): any {
  ...
  const formatDate = (dateField: { type: number; value: number[] } | undefined): string => {
    if (!dateField || !dateField.value || !Array.isArray(dateField.value) || dateField.value.length === 0) {
      return '';
    }
